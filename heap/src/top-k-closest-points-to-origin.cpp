...
std::vector<std::vector<int>> kClosest2(std::vector<std::vector<int>>& points, int K) {
    std::partial_sort(points.begin(), points.begin() + K, points.end(), [](auto& p, auto& q) {
...
std::vector<std::vector<int>> kClosest3(std::vector<std::vector<int>>& points, int K) {
    std::nth_element(points.begin(), points.begin() + K - 1, points.end(), [](auto& p, auto& q) { /// nth element is at K-1 th index.
        return p[0] * p[0] + p[1] * p[1] < q[0] * q[0] + q[1] * q[1];
    });
